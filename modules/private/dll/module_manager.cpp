...
namespace hana
{
	const char8_t* IDynamicModule::get_meta_data() const {
		...
		return shared_library.get<const char8_t*>(meta_symbol.data());
	}
...
namespace hana
{
	...
	IModule* ModuleManagerImpl::LoadModule(HString name, const bool shared, const bool load, const int argc, char8_t** argv) {
		...
		if (shared) {
			...
			if (!is_proc_mod && !func) {
				...
				if (!hotfix) {
					if (!shared_library.load((root / filename).u8string().data())) {
						...
						func = static_cast<IDynamicModule*(*)()>(shared_library.get_symbol(init_func_name.data()));
					}
				} else if (LoadHotfixModule(&shared_library, hash, std::move(filename))) {
			...
			reinterpret_cast<IDynamicModule*>(module)->shared_library = std::move(shared_library);
		}
		// LoadStaticModule
		else {
	...
	void ModuleManagerImpl::DestroyModules() {
		for (auto&& [module, hash]: destruction) {
			...
			module->on_unload();
			if (module->is_shared()) {
				SharedLibrary lib = std::move(reinterpret_cast<IDynamicModule*>(module)->shared_library);
				delete module;
			} else {
				delete module;
			}
	...
	bool ModuleManagerImpl::update() {
		for (auto hash: hotfixs) {
			...
			if (std::filesystem::last_write_time(ctx.path) > ctx.timestamp) {
				...
				SharedLibrary shared_library = std::move(this_module->shared_library);
				delete this_module;

				if (!LoadHotfixModule(&shared_library, hash, get_file_name(this_name))) {
					return false;
				}

				const HString init_name = HString::concat(HANA_MAKE_UTF8(HANA_DYNAMIC_MODULE_CREATION_PRIFIX), this_name);
				auto* func = static_cast<IDynamicModule*(*)()>(shared_library.get_symbol(init_name.c_str()));
				...
				new_module->shared_library = std::move(shared_library);
...
namespace hana
{
	...
	void ModuleManagerImpl::set_root(const char8_t* path) {
		root = path ? path : std::move(std::filesystem::current_path());
	}
	...
	IModule* ModuleManagerImpl::get_module(const char8_t* name) noexcept {
...
namespace hana
{
	ModuleManagerImpl::GraphvizDumper::GraphvizDumper(): registry(graphviz::Registry::create()) {
		graph = registry->create_graph(u8"modules");
	}

	graphviz::Node* ModuleManagerImpl::GraphvizDumper::dfs(IModule* module) {
		if (auto&& iter = visited.find(module); iter != visited.end()) {
			return iter->second;
		}

		using namespace graphviz;

		auto v = registry->create_node(module->name);
		if (module->name.starts_with(u8"Hana")) {
			v
				->add_attribute(NodeAttr::fillcolor_, u8"lightgreen")
				->add_attribute(NodeAttr::style_, u8"filled")
				->add_attribute(NodeAttr::shape_, "polygon")
				->add_attribute(NodeAttr::sides_, "4");
		} else {
			v
				->add_attribute(NodeAttr::fillcolor_, u8"lightblue")
				->add_attribute(NodeAttr::style_, u8"filled");
		}
	...
	void ModuleManagerImpl::GraphvizDumper::dump(const char8_t* path) {
		for (auto sz: instance().roots) {
			dfs(instance().modules_map[sz]);
		}
...
namespace hana
{
	...
	IModule* ModuleManager::get_module(const char8_t* name) noexcept {
		return ModuleManagerImpl::instance().get_module(name);
	}

	Graph* ModuleManager::get_dependency_graph() noexcept {
		return ModuleManagerImpl::instance().get_dependency_graph();
	}

	void ModuleManager::dump_graphviz(const char8_t* path) {
		ModuleManagerImpl::GraphvizDumper().dump(path);
	}
