...
namespace hana::fmt
{
	...
	inline const char8_t* measure_string_prefix(const HStringView value, int& width) {
	...
	appender write_aligned(appender out, const int width, const T& specs, const Align default_align, Fn&& func) {
		...
		const HStringView fill_char{specs.fill_, specs.fill_length_};
	...
	appender write_integral(appender out, const T value, basic_format_specs specs, const std::locale* loc) {
		...
		auto writer = [&, end = end](appender out_) {
			out_ = fmt::write_sign(out_, specs.sgn_, value < T{0});
			...
			if (separators > 0) {
				return fmt::write_separated_integer(
					buffer_start,
					end,
					groups,
		...
		return fmt::write_aligned(out, width, specs, Align::right, writer);
	}
}
...
namespace hana::fmt
{
	...
	inline appender write(appender out, const HStringView value) {
	...
	appender write(appender out, const T value, const basic_format_specs& specs, const std::locale* loc) {
		return fmt::write_integral(out, value, specs, loc);
	}
	...
	inline appender write(appender out, const bool value, basic_format_specs specs, const std::locale* loc) {
		...
		if (specs.localized_) {
			...
			return write(out, HStringView(reinterpret_cast<const char8_t*>(sv.data()), sv.size()), specs, loc);
		}

		return write(out, value ? u8"true" : u8"false", specs, loc);
	}

	inline appender write_escaped(appender out, HStringView value, char8_t delim) {
		...
		while (first != last) {
			...
			if (ch == '\t') {
				...
				*out++ = delim;
				...
				char32_t decoded_ch;
				const auto [next, is_usv] = unicode::decode_utf(first, last, decoded_ch);
	...
	inline appender write_escaped(appender out, HStringView value, const basic_format_specs& specs, char8_t delim) {
		...
		HString temp;
		if (true) {
			iterator_buffer<std::back_insert_iterator<HString>, buffer_traits> buf(std::back_inserter(temp));
		...
		return write_aligned(out, width, specs, Align::left, [&temp, last](appender out_) {
			const std::basic_string_view sv{temp.data(), last};
			return write(out_, HStringView{sv.data(), sv.size()});
		});
	}

	inline appender write(appender out, const char8_t value, const basic_format_specs& specs, const std::locale* loc) {
		...
		if (specs.type_ == '?') {
			return write_escaped(out, HStringView{&value, 1}, specs, '\'');
		}

		return write(out, HStringView{&value, 1}, specs, loc);
	}
	...
	appender write(appender out, const T value, const basic_format_specs& specs, const std::locale* loc) {
		...
		auto writer = [&](appender out_) {
			out_ = fmt::write_sign(out_, sgn, is_negative);
	...
	appender write(appender out, const char8_t* value, const basic_format_specs& specs, const std::locale* loc) {
		return write(out, HStringView{value}, specs, loc);
	}

	appender write(appender out, const HStringView value, const basic_format_specs& specs, const std::locale*) {
		...
		return write_aligned(out, width, specs, Align::left, [=](appender out_) {
			const std::basic_string_view sv{value.data(), last};
			return write(out_, HStringView{sv.data(), sv.size()});
		});
	}
}
