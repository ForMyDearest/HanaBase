...
#define HANA_IMPLEMENT_STATIC_MODULE(ModuleName, ModuleImplClass)							\
	inline static const hana::StaticallyLinkedModuleRegistrant HanaRegistrant##ModuleName(	\
		...
		[] { return static_cast<hana::IStaticModule*>(new ModuleImplClass); }				\
	);
...
#define HANA_IMPLEMENT_DYNAMIC_MODULE(ModuleName, ModuleImplClass)													\
	extern "C" HANA_EXPORTS hana::IDynamicModule* HANA_JOIN(HANA_DYNAMIC_MODULE_CREATION_PRIFIX, ModuleName)() {	\
		return static_cast<hana::IDynamicModule*>(new ModuleImplClass);												\
	}

#define HANA_IMPLEMENT_MODULE_SUBSYSTEM(ModuleName, Subsystem)								\
	inline static hana::ModuleSubsystemRegistrant HanaRegistrant##ModuleName##Subsystem(	\
		HANA_MAKE_UTF8(ModuleName),															\
		HANA_MAKE_UTF8(HANA_UNIQUE_VARNAME(Subsystem)),										\
		[] { return static_cast<hana::IModuleSubsystem*>(new Subsystem); }					\
	);
