...
namespace hana::internal
{
	...
	template<typename T>
	constexpr uint32_t read32(const T* v) noexcept {
	...
	struct xxh32 {
		...
		template<typename T>
		static constexpr uint32_t hash(const T* input, const uint32_t len, const uint32_t seed) noexcept {
	...
	private:
		...
		static constexpr uint32_t round(uint32_t acc, const uint32_t input) noexcept {
			return std::rotl(acc + (input * PRIME2), 13) * PRIME1;
		}

		static constexpr uint32_t avalanche_step(const uint32_t h, const int rshift, const uint32_t prime) noexcept {
			return (h ^ (h >> rshift)) * prime;
		}

		// Mixes all bits to finalize the hash.
		static constexpr uint32_t avalanche(const uint32_t h) noexcept {
			return avalanche_step(avalanche_step(avalanche_step(h, 15, PRIME2), 13, PRIME3), 16, 1);
		}

		template<typename T>
		static constexpr uint32_t fetch32(const T* p, const uint32_t v) noexcept {
		...
		template<typename T>
		static constexpr uint32_t h16bytes(const T* p, uint32_t len, const uint32_t seed) noexcept {
...
namespace hana
{
	...
	public:
		...
		template<typename T>
		[[nodiscard]] static constexpr uint32_t xxhash32(const T* input, size_t length, uint32_t seed = default_seed) noexcept {
		...
		[[nodiscard]] static constexpr uint32_t xxhash32(const Bytes& input, uint32_t seed = default_seed) noexcept {
			return XXHash::xxhash32(std::data(input), internal::constexpr_xxh3::bytes_size(input), seed);
		}

		template<typename T>
		[[nodiscard]] static constexpr uint64_t xxhash64(const T* input, size_t length) noexcept {
			if constexpr (internal::constexpr_xxh3::ByteType<T>) {
				if (std::is_constant_evaluated()) {
		...
		template<typename T>
		[[nodiscard]] static constexpr uint64_t xxhash64(const T* input, size_t length, uint64_t seed) noexcept {
			if constexpr (internal::constexpr_xxh3::ByteType<T>) {
				if (std::is_constant_evaluated()) {
		...
		template<internal::constexpr_xxh3::BytesType Bytes>
		[[nodiscard]] static constexpr uint64_t xxhash64(const Bytes& input) noexcept {
			return XXHash::xxhash64(std::data(input), internal::constexpr_xxh3::bytes_size(input));
		}

		template<internal::constexpr_xxh3::BytesType Bytes>
		[[nodiscard]] static constexpr uint64_t xxhash64(const Bytes& input, uint64_t seed) noexcept {
			return XXHash::xxhash64(std::data(input), internal::constexpr_xxh3::bytes_size(input), seed);
		}

		template<typename T>
		[[nodiscard]] static constexpr size_t xxhash(const T* input, size_t length) noexcept {
		...
		template<typename T>
		[[nodiscard]] static constexpr size_t xxhash(const T* input, size_t length, size_t seed) noexcept {
			if constexpr (sizeof(size_t) == 4) {
		...
		template<internal::constexpr_xxh3::BytesType Bytes>
		[[nodiscard]] static constexpr size_t xxhash(const Bytes& input) noexcept {
			return XXHash::xxhash(std::data(input), internal::constexpr_xxh3::bytes_size(input));
		}

		template<internal::constexpr_xxh3::BytesType Bytes>
		[[nodiscard]] static constexpr size_t xxhash(const Bytes& input, size_t seed) noexcept {
			return XXHash::xxhash(std::data(input), internal::constexpr_xxh3::bytes_size(input), seed);
		}

	private:
		HANA_BASE_API static uint32_t _xxhash32(const void* input, size_t length, uint32_t seed) noexcept;
		HANA_BASE_API static uint64_t _xxhash64(const void* input, size_t length) noexcept;
		HANA_BASE_API static uint64_t _xxhash64(const void* input, size_t length, uint64_t seed) noexcept;
	...
	[[nodiscard]] constexpr size_t hash_combine(const size_t seed, const size_t combine, Ts&&... rest) noexcept {
		...
		if constexpr (sizeof...(Ts) == 0) {
			...
			return hash_combine(res, std::forward<Ts>(rest)...);
		}
	}
}
...
namespace hana
{
	...
	template<>
	struct Hash<HStringView> {
		constexpr size_t operator()(HStringView value) const noexcept {
	...
	template<>
	struct Hash<HString> {
		size_t operator()(const HString& value) const noexcept {
			return XXHash::xxhash(value);
		}
	};

	template<typename T, typename Traits>
	struct Hash<std::basic_string_view<T, Traits>> {
		constexpr size_t operator()(std::basic_string_view<T, Traits> value) const noexcept {
			return XXHash::xxhash(value);
		}
	};

	template<typename T, typename Traits, typename Allocator>
	struct Hash<std::basic_string<T, Traits, Allocator>> {
		size_t operator()(const std::basic_string<T, Traits, Allocator>& value) const noexcept {
			return XXHash::xxhash(value);
		}
	};
	...
	struct Hash<Char[N]> {
		constexpr size_t operator()(const Char (&value)[N]) const noexcept {
			return XXHash::xxhash(value, N - 1);
		}
	};

	template<is_char_v Char>
	struct Hash<const Char*> : Hash<std::basic_string_view<Char>> {};

	template<is_char_v Char>
	struct Hash<Char*> : Hash<std::basic_string_view<Char>> {};
}
