...
struct SPTRIntrusiveTests {
	...
	static auto make_intrusive(Args&&... args) {
		return hana::intrusive_ptr{new T{std::forward<Args>(args)...}};
	}
};

struct TestObject : hana::intrusive_base {
	enum Status {
		Uninitialized,
		Created,
		Hosted,
...
TEST_CASE_FIXTURE(SPTRIntrusiveTests, "CopyIntrusive") {
	...
	[&] {
		...
		hana::intrusive_ptr<TestObject> mCopy(mTestObject);
...
TEST_CASE_FIXTURE(SPTRIntrusiveTests, "CastIntrusive") {
	...
	hana::intrusive_ptr<TestObject> pP(pC);
	// HBase::intrusive_ptr<TestSon> pCC(HBase::static_pointer_cast<TestSon>(pP));
	// CHECK_EQ(pCC->use_count(), 3);
	//
	// HBase::intrusive_ptr<TestObject> pCC2(pCC);
	// CHECK_EQ(pCC->use_count(), 4);
	//
	// HBase::intrusive_ptr<TestObject> pCC3(std::move(pCC));
