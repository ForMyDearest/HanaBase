...
TEST_CASE("Test HStringView") {
	...
	SUBCASE("compare") {
		...
		HStringView a{text_a};     // 🐓鸡ĜG
		HStringView b{text_a, 9};  // 🐓鸡Ĝ
		HStringView c{text_b};     // 🐓鸡Ĝ
		HStringView d{text_a};     // 🐓鸡ĜG
		HStringView e{text_c + 6}; // 🐓
		HStringView f{text_a, 4};  // 🐓
		HStringView empty{};
		HStringView with_value_empty{text_a, 0};
	...
	SUBCASE("str get") {
		constexpr char8_t text[] = u8"🐓鸡ĜG";
		HStringView a{text};
		HStringView b{text, 6};
		HStringView c{text + 1, 6};
	...
	SUBCASE("sub view") {
		HStringView view{u8"🐓鸡ĜG"};
		HStringView sub_a = view.first_view(4);
		HStringView sub_b = view.subview(0, 4);
		HStringView sub_c = view.last_view(6);
		HStringView sub_d = view.subview(4);
		HStringView sub_e = view.subview(4, 3);

		CHECK_EQ(sub_a, sub_b);
		CHECK_EQ(sub_a, HStringView{ u8"🐓" });
		CHECK_EQ(sub_c, sub_d);
		CHECK_EQ(sub_c, HStringView{ u8"鸡ĜG" });
		CHECK_EQ(sub_e, HStringView{ u8"鸡" });
	}

	SUBCASE("find") {
		HStringView view{u8"This 🐓 is 🐓 a good 🐓 text 🐓"};
		HStringView find_view = u8"🐓";
	...
	SUBCASE("contains & count") {
		HStringView view{u8"This 🐓 is 🐓 a good 🐓 text 🐓"};
		HStringView none_view = u8"🐔";
		UTF8Seq none_seq = {u8"🐔", 4};
		HStringView has_view = u8"🐓";
	...
	SUBCASE("starts_with & ends_with") {
		HStringView view_a{u8"This is a good text"};
		HStringView view_b{u8"🐓🐓🐓 This is a good text 🐓🐓🐓"};
	...
	SUBCASE("remove prefix & suffix") {
		HStringView view_a{u8"🐓🐓🐓 This is a good text 🐓🐓🐓"};
		HStringView view_b{u8" This is a good text 🐓🐓🐓"};
		HStringView view_c{u8"🐓🐓🐓 This is a good text "};
		HStringView view_d{u8" This is a good text "};
		HStringView view_e{u8"🐓🐓 This is a good text 🐓🐓🐓"};
		HStringView view_f{u8"🐓🐓🐓 This is a good text 🐓🐓"};
		HStringView view_g{u8"🐓🐓 This is a good text 🐓🐓"};
	...
	SUBCASE("trim") {
		HStringView trim_view{u8" \tThis is a good text\t "};
		HStringView trim_view_start{u8"This is a good text\t "};
		HStringView trim_view_end{u8" \tThis is a good text"};
		HStringView trim_view_full{u8"This is a good text"};
		HStringView trim_view_start_seq{u8"\tThis is a good text\t "};
		HStringView trim_view_end_seq{u8" \tThis is a good text\t"};
		HStringView trim_view_full_seq{u8"\tThis is a good text\t"};
		HStringView trim_view_empty{u8" \t\t "};
		HStringView empty{};
		...
		HStringView custom_trim_view{u8"🐓🐓🐓 \tThis is a good text\t 🐓🐓🐓"};
		HStringView custom_trim_view_start{u8"This is a good text\t 🐓🐓🐓"};
		HStringView custom_trim_view_end{u8"🐓🐓🐓 \tThis is a good text"};
		HStringView custom_trim_view_full{u8"This is a good text"};
		HStringView custom_trim_view_start_seq{u8" \tThis is a good text\t 🐓🐓🐓"};
		HStringView custom_trim_view_end_seq{u8"🐓🐓🐓 \tThis is a good text\t "};
		HStringView custom_trim_view_full_seq{u8" \tThis is a good text\t "};
		HStringView custom_trim_view_empty{u8"🐓🐓🐓 \t\t 🐓🐓🐓"};

		HStringView custom_view = u8"🐓 \t";
	...
	SUBCASE("trim invalid") {
		HStringView view{u8"🐓🐓🐓 This is a good text 🐓🐓🐓"};
		HStringView bad = view.subview(1, view.size() - 2);
		HStringView trim_start = bad.subview(3);
		HStringView trim_end = bad.subview(0, bad.size() - 3);
		HStringView trim_full = u8"🐓🐓 This is a good text 🐓🐓";
	...
	SUBCASE("partition") {
		SUBCASE("view partition") {
			// util
			HStringView view{u8"This is a 🐓 good text"};
			HStringView left{u8"This is a "};
			HStringView mid{u8"🐓"};
			HStringView right{u8" good text"};
			...
			// not found
			HStringView not_found_view{u8"This is a good text"};
			...
			// first_view
			HStringView first_view{u8"🐓 This is a good text"};
			auto [first_left, first_mid, first_right] = first_view.partition(u8"🐓");
			CHECK(first_left.empty());
			CHECK_EQ(first_mid, u8"🐓");
			CHECK_EQ(first_right, HStringView{ u8" This is a good text" });

			// last_view
			HStringView last_view{u8"This is a good text 🐓"};
			auto [last_left, last_mid, last_right] = last_view.partition(u8"🐓");
			CHECK_EQ(last_left, HStringView{ u8"This is a good text " });
	...
	SUBCASE("text index") {
		...
		HStringView bad_view = view.subview(1, view.size() - 5);
